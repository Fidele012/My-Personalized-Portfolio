    ...
    <main className="w-full min-h-screen text-[#1a1a1a] selection:bg-cyan-200">
      ...
      <section className="pt-40 px-6 md:px-12 pb-20 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
            ...
            {/* --- LEFT: PHOTO CARD (The Backdrop) --- */}
            <motion.div 
                initial={{ opacity: 0, x: -50 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 1 }}
                // LAYER 0: Stays at the back
                className="lg:w-[40%] w-full sticky top-32 z-0"
            >
                {/* 3D Card Effect */}
            ...
            {/* --- RIGHT: NARRATIVE & SKILLS (The Floating Content) --- */}
            <motion.div 
                initial={{ opacity: 0, y: 50 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.3, duration: 1 }}
                // LAYER 20: This Z-index ensures this content slides OVER the sticky image
                className="lg:w-[60%] flex flex-col gap-12 relative z-20"
            >
                {/* Bio */}
                {/* Added 'backdrop-blur' so if it floats over the image, it looks nice */}
                ...
                {/* Skills Grid */}
                {/* Because of 'z-20' on the parent, these will now float ON TOP of the picture when you scroll */}
                <div>
